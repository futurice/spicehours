(function() {
  ...
  function formatDuration(duration) {
    ...
    if (Math.floor(duration / 3600) !== 0 || duration % 3600 === 0) {
      str += Math[duration >= 0 ? 'floor' : 'ceil'](duration / 3600);
      str += ' hours';
    }
    if (duration % 3600 !== 0) {
      if (str.length > 0) str += ' ';
  ...
  class EventItem extends React.Component {
    ...
    render() {
      ...
      var date = dateFormatter.format(
        event.block ? new Date(event.block.timestamp * 1000) : new Date()
      );
      var confirm = latestBlock && event.block
        ? ' ' + (latestBlock.number - event.block.number) + ' confirmations'
        : null;
      var eventdiv = React.createElement(
        'div',
        {},
      ...
      if (event.event === 'MarkHours') {
        ...
        eventdiv = React.createElement(
          'div',
          {},
          name +
            ' marked ' +
            formatDuration(event.args.duration) +
            ' to project ',
          title,
        ...
        eventdiv = React.createElement(
          ...
          React.createElement(
            'a',
            { href: '/payrolls/' + event.args.payroll },
            'payroll'
          ),
  ...
  class TopContent extends React.Component {
    ...
    render() {
      ...
      var name =
        ...
        '(unknown)';
      var status = profile.unpaidPercentage !== undefined &&
        profile.unpaidPercentage !== 0
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          ' ' + name,
          React.createElement('br'),
          React.createElement('span', {}, 'Your employee status is'),
          ' ' + status,
          React.createElement('br'),
          React.createElement('span', {}, 'This period you have marked'),
          ' ' + duration,
          React.createElement('br')
        ),
  ...
  class HoursForm extends React.Component {
    constructor() {
      ...
      this.sendMarking = this.sendMarking.bind(this);
    ...
    isTimeValid() {
      return this.state.minutes >= 0 && this.state.minutes < 60 &&
    ...
    isTitleValid() {
      return (this.state.title.length > 0 && this.state.title.length <= 32) ||
        urlRegex.test(this.state.title);
    }
    ...
    sendMarking() {
      if (!this.isTitleValid()) {
        return this.props.errorCallback(
          'Invalid title for hour marking, use between 1 and 32 ' +
          'characters or an URL'
        );
      }

      if (!this.isTimeValid) {
        return this.props.errorCallback(
          'Invalid time spent for hour marking, must not be zero'
        );
      }
